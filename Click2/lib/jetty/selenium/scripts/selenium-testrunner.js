...
function onSeleniumLoad() {
    if (window.location.toString().indexOf("group") != -1) {
		var group = window.location.toString().substring(window.location.toString().indexOf("group") + 6);
		$('suiteReplace').innerHTML = '<iframe name="testSuiteFrame" id="testSuiteFrame" src="selenify?group=' + group + '" application="yes"></iframe>'
	} else {
		$('suiteReplace').innerHTML = '<iframe name="testSuiteFrame" id="testSuiteFrame" src="selenify" application="yes"></iframe>'
	}

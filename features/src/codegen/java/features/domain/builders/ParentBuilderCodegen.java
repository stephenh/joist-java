...
public abstract class ParentBuilderCodegen extends AbstractBuilder<Parent> {
  ...
  public PrimitivesBuilder newPrimitives() {
    return Builders.aPrimitives().parent((ParentBuilder) this);
  }

  public List<PrimitivesBuilder> primitivess() {
    List<PrimitivesBuilder> b = new ArrayList<PrimitivesBuilder>();
    for (Primitives e : get().getPrimitivess()) {
      b.add(Builders.existing(e));
    }
    return b;
  }

  public PrimitivesBuilder primitives(int i) {
    return Builders.existing(get().getPrimitivess().get(i));
  }

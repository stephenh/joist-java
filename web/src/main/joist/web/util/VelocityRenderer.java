...
public class VelocityRenderer {
    ...
    public static void render(Control control, HtmlWriter w) {
        String template = CurrentContext.get().getClickConfig().getPageResolver().getTemplateFromPage(control.getClass().getName());
        Log.debug("Rendering {} for {}", template, control);
        try {
            VelocityEngine ve = CurrentContext.get().getClickConfig().getVelocityEngine();
